...
sim.newEpisode(['17DRP5sb8fy'],['3577de361e1a46b1be544d37731bfde6'],[0],[0])
# Define goal information
# goal_viewpoint = "e34dcf54d26a4a95869cc8a0c01cd2be"
# goal_viewpoint="e34dcf54d26a4a95869cc8a0c01cd2be"
goal_viewpoint = "c341c46acf7044d1a712d622cbc94a27"
...
# goal = ("Turn around and walk into the bedroom. Wait near the bed.")
# goal = ("Walk through the sitting room.  Stop right after the open door.")
goal = ("Go straight then turn right. Pass the bed then turn left pass the shower on the right and the sink on the right and wait by the toilet. ")
...
def get_lookahead_desc(sim, locations, viewpoint_descs, current_viewpoint, nav_graph):
    ...
    for loc in locations[1:]:
        ...
        viewpoint_descs[loc.viewpointId] = desc_response
...
while True:
    ...
    # Serialize the navigation graph into text
    # graph_text = graph_to_text(nav_graph)
    graph_text = graph_to_text_shortened(nav_graph,locations)
    # graph_text = graph_text.replace('\u201c', '"').replace('\u201d', '"').replace('\u2014', '--').replace('\u2019',"'")
    graph_text = unidecode(graph_text)
    #graph_text ki copy then usko prune
    print("\n---------Generated Graph:", graph_text,"-----------\n")
    print("Nav History:"," -> ".join(path_history))
    # Make a decision by sending context (current description, viewpoints, goal, history, graph) to the server.
    ...
    chosen_vp = best_viewpoint['v_id']
    print(f"\nChosen viewpoint id: ",best_viewpoint)

    # Check if the agent is entering a cycle (visiting the same viewpoint again)
    if False:
    # if chosen_vp in visited:
        print(f"\nCycle detected! Agent is looping back to {chosen_vp}")

        # Call the LLM to analyze the cycle and suggest a new viewpoint
        cycle_check_url = "http://host.docker.internal:3000/detect_cycle"
        cycle_response = requests.post(cycle_check_url, json={
            "current_viewpoint": current_vp_id,
        ...
        reasoning = cycle_data.get("reasoning", "No explanation provided")
        suggested_vp = cycle_data.get("alternative_viewpoint", chosen_vp)  # Default to original if no alternative

        print(f"\nLLM Explanation for Cycle: {reasoning}")

        if suggested_vp != chosen_vp:
            print(f"\nLLM suggests an alternative viewpoint: {suggested_vp}")
            chosen_vp = suggested_vp  # Use the alternative viewpoint
        else:
            print(f"\nLLM suggested the same previous viewpoint")

    # Proceed with navigation
    visited.append(chosen_vp)
    best_viewpoint['vid'] = chosen_vp
    
    print(f"Best viewpoint: {best_viewpoint}")

    # Navigate to the chosen viewpoint and update the graph with the new edge.
    try:
        if len(viewpoints_to_exclude) >= 2:
            viewpoints_to_exclude.pop(0)
        viewpoints_to_exclude.append(best_viewpoint['v_id'])
        prev_vp_id = current_vp_id

        if not path_history:
            path_history.append(current_vp_id)

        best_vp = best_viewpoint['v_id']

        path_history.append(best_vp)

        next_desc = ""
        if best_vp in viewpoint_descs:
            next_desc = viewpoint_descs[best_vp].get("unified_description", "")
        update_graph(nav_graph, prev_vp_id, best_vp, next_desc)
        current_vp_id = best_vp
        sim.newEpisode([EPISODE_NUMBER], [best_vp], [0], [0])
